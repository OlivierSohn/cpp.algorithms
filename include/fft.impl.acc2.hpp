...
struct UnwrapFrequenciesRealFBins<accelerate2::Tag, CONTAINER> {
  ...
  static auto run_half_alt(CONTAINER const & const_container, int N, std::vector<complex<T>> & res) {
    auto observed = const_cast<CONTAINER &>(const_container).get_hybrid_split();
    res.clear();
    res.resize(required_capacity_half_spectrum(N),
               {0,0});
    res[0] = {
      observed.realp[0],
      0
  ...
  static auto run_half_alt_sqmag(CONTAINER const & const_container, int N, std::vector<T> & res) {
    auto observed = const_cast<CONTAINER &>(const_container).get_hybrid_split();
    res.clear();
    res.reserve(required_capacity_half_spectrum(N));
    
    res.push_back(observed.realp[0] * observed.realp[0]);
    
    // The factors 4. are to take into account the symmetric negative-frequencies spectrum,
  ...
  static std::size_t required_capacity_half_spectrum(int const fft_length) {
    return 1 + fft_length/2;
  }
  static std::size_t required_capacity_full_spectrum(int const fft_length) {
    return fft_length;
  }
};
