...
template<class F, class TUPLE1, class TUPLE2, std::size_t...Is>
void for_each_zip(TUPLE1 & tuple1, TUPLE2 & tuple2, F && func, std::index_sequence<Is...>){
  using expander = int[];
  (void)expander { 0, ((void)func(std::get<Is>(tuple1),
                                  std::get<Is>(tuple2)), 0)... };
}

template<class F, class TUPLE1, class TUPLE2>
void for_each_zip(TUPLE1 & tuple1, TUPLE2 & tuple2, F && func){
  static_assert(std::tuple_size_v<TUPLE1> == std::tuple_size_v<TUPLE2>);
  for_each_zip(tuple1, tuple2, func, std::make_index_sequence<std::tuple_size<TUPLE1>::value>());
}
