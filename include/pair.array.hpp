  ...
  public:
    ...
    A const * firsts() const {
      if constexpr (order == Order::As_Bs) {
        ...
        return reinterpret_cast<A const*>(arr.buf() + arr.size());
      }
    }
    ...
    B const * seconds() const {
      if constexpr (order == Order::Bs_As) {
        ...
        return reinterpret_cast<B const*>(arr.buf() + arr.size());
      }
    }
    ...
    A& corresponding(B const &b) {
      return firsts()[index(b)];
    }

    B& corresponding(A const &a) {
      return seconds()[index(a)];
    }

    int index(A const &a) const {
      return &a - firsts();
    }

    int index(B const &b) const {
      return &b - seconds();
    }
