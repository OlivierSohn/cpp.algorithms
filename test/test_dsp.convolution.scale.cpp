...
TEST(ConvolutionScale, iterateScales_nolimitRepeats) {
...
TEST(ConvolutionScale, iterateScales_limitRepeats) {
    using namespace imajuscule;
    
    {
        int const firstSz = 4;
        int const nCoeffs = 100*firstSz;
        std::set<std::vector<Scaling>> results;
        ScalingsIterator it{
            firstSz,
            nCoeffs,
            {10} // limit the number of repetitions
        };
        int count = 0;
        it.forEachScaling([&count, &results](auto const & v){
            results.emplace(v);
